...
export default function Lanyard({
  ...
  return (
    <div className="relative z-0 w-full h-screen flex justify-center items-center transform scale-100 origin-center bg-white">
      <Canvas
        camera={{ position: position, fov: fov }}
        dpr={[1, isMobile ? 1.5 : 2]}
        gl={{ alpha: transparent, antialias: true }}
        onCreated={({ gl }) => gl.setClearColor(new THREE.Color(0x18181b), transparent ? 0 : 1)}
      >
...
function Band({
  ...
  // Unconditional asset hooks at the absolute top level
    ...
    if (fixed.current && j1.current && j2.current && j3.current && band.current) {
      [j1, j2].forEach(ref => {
        ...
        ref.current.lerped.lerp(
          ...
          actualDelta * (minSpeed + clampedDistance * (maxSpeed - minSpeed))
        );
      });
          ...
          >
            <mesh geometry={nodes.card.geometry}>
              <meshPhysicalMaterial
                map={cardMap}
                color={!cardMap ? "#e4e4e7" : undefined}
                map-anisotropy={16}
